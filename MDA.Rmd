...
prop.test(x = c(sum(summaryTestsNoM$prep <= .05, na.rm = T), sum(summaryTestsNoM$pcalDir <= .05, na.rm = T)), n = c(sum(!is.na(summaryTestsNoM$prep)), sum(!is.na(summaryTestsNoM$pcalDir))))

# If we further consider all p-values, reported in threshold and exact format and compare the proportion of test smaller or equal to .05 the dicrepancy is even larger: 78% of the reported p-values are p <= .05 and only 71% of the recalculated p-values.
...
# Figure X illustrates how the discrepancy in reported and recalculated p-values changed between 2008 and 2020. Thereafter, the reported p-values are more likely to be correctly reported in 2020 compared to 2008.
...
print(sum(aggrsummaryTestsTwo$Prop[aggrsummaryTestsTwo$year == i + 7]))
...
ERRs =
cbind(rbind(
  quantile(z8$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z9$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z10$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z11$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z12$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z13$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z14$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z15$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z16$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z17$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z18$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z19$coefficients_boot[,1], prob = c(.025, .5,.975)),
  quantile(z20$coefficients_boot[,1], prob = c(.025, .5,.975)),
  ...
  quantile(z20d$coefficients_boot[,1], prob = c(.025, .5,.975))), "year" = rep(8:20,2), "direction" = rep(c("two-sided", "one and two-sided"),each = 13))
...
ggplot(ERRs, aes( y = `50%`, x = year, ymin = `2.5%`,ymax = `97.5%`, linetype = direction, group = direction))+
  geom_line(position = position_dodge(width = .2))+
  ylab("Expected replication rate (95%CI)")+
  scale_x_continuous(breaks = 8:20, n)+
  geom_pointrange(position = position_dodge(width = .2))


zD = zcurve(summaryTestsNoM$zscoreD, bootstrap = F)


plot(z8, CI = TRUE, annotation = TRUE, main = "2008")


plot(zD, CI = TRUE, annotation = TRUE, main = "2008-2020")
